use crate::{
    ...
    services::{
        ...
        compositor::{self, CompositorCommand, CompositorService, CompositorState},
    },
    theme::AshellTheme,
};
use iced::{
...
fn icons_enabled(config: &WorkspacesModuleConfig) -> bool {
    config.indicator_format == WorkspaceIndicatorFormat::NameAndIcons
        && (!config.window_icons.is_empty() || config.default_window_icon.is_some())
}
...
fn resolve_workspace_icons(
    ...
    for class in window_classes {
        let icon = config
            .window_icons
            .iter()
            .find(|(pattern, _)| class.contains(pattern.as_str()))
            .map(|(_, icon)| icon)
...
impl Workspaces {
    pub fn new(config: WorkspacesModuleConfig) -> Self {
        compositor::set_collect_window_classes(icons_enabled(&config));
        Self {
            config,
            service: None,
    ...
    pub fn update(&mut self, message: Message) -> iced::Task<Message> {
        match message {
            ...
            Message::ConfigReloaded(cfg) => {
                compositor::set_collect_window_classes(icons_enabled(&cfg));
                self.config = cfg;
                self.recalculate_ui_workspaces();
                iced::Task::none()
            }
                                ...
                                button(content)
                                    ...
                                    .width(if dynamic_width {
                                        ...
                                        match w.displayed {
                                            ...
                                            Displayed::Visible => Length::Fixed(theme.space.lg as f32),
                                            Displayed::Hidden => Length::Fixed(theme.space.md as f32),
                                        }
                                    })
